...
app.get("/languages", async (req: Request, res: Response) => {
	...
	try {
		...
		console.log("GET /languages success, returning data")
		res.json(data)
	} catch (error: any) {
		console.error("Error proxying languages:", error)
		res.status(500).json({ message: "Failed to fetch languages from Judge0" })
	}
})

// Proxy for Judge0 Submissions
app.post("/submissions", async (req: Request, res: Response) => {
	console.log("POST /submissions requested, body:", req.body)
	try {
		const { source_code, language_id, stdin } = req.body

		// Encode inputs to base64 to prevent UTF-8 transmission errors
		...
		// Decode outputs from base64 back to UTF-8
		if (data.stdout) {
			data.stdout = Buffer.from(data.stdout, "base64").toString("utf8")
		}
		if (data.stderr) {
			data.stderr = Buffer.from(data.stderr, "base64").toString("utf8")
		}
		if (data.compile_output) {
			data.compile_output = Buffer.from(data.compile_output, "base64").toString("utf8")
		}
		if (data.message) {
			data.message = Buffer.from(data.message, "base64").toString("utf8")
		}

		console.log("POST /submissions response status:", response.status, "decoded data:", data)
		res.status(response.status).json(data)
	} catch (error: any) {
		console.error("Error proxying submissions:", error)
		res.status(500).json({ message: "Failed to run code via Judge0" })
	}
})
