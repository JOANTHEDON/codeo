...
const ViewButton = ({ viewName, icon, disabled = false }: ViewButtonProps) => {
    ...
    return (
        <div className="relative flex flex-col items-center">
            <button
                ...
                className={`${buttonStyles.base} ${disabled ? 'opacity-50 cursor-not-allowed' : buttonStyles.hover}`}
                disabled={disabled}
                {...(showTooltip && {
                    "data-tooltip-id": `tooltip-${viewName}`,
                    "data-tooltip-content": disabled ? `${viewName} (Feature in Development)` : viewName,
                })}
            >
                <div className="flex items-center justify-center">{icon}</div>
                {/* Show dot for new message in chat View Button */}
                {viewName === VIEWS.CHATS && isNewMessage && !disabled && (
